...
以下是此套件的優化項目，針對 Delta PLC 在 Modbus TCP 通訊中的一些常見限制進行了改善：
...
1. **跳址問題**  
   ...
   此情況限制了批量讀取和批量寫入操作的實現，對於大範圍操作效率造成影響。本套件針對此問題進行了優化。

2. **Word/Dword 符號處理**  
   增加對 Word 和 Dword 讀取/寫入的正負符號處理，以符合不同數據類型的需求。

3. **Dword 計算**  
   支援 Dword 讀取與寫入的計算，改善 Dword 數據的處理效率。

4. **KeepAlive 長連線**  
   Delta PLC 預設在連線閒置超過 120 秒未接收到信號時會自動斷線。例如：  
   如果用戶的應用程式設定在每 130 秒交替閃爍指示燈，連線將會因為超時被斷開。  
   為解決此問題，本套件加入了 KeepAlive 機制以保持長連線。
